import { DTF_COLORS, DTF_SIZES, DTF_SHIRT_MATERIALS } from "./dtf-catalog.js";
...
export function calculateLine(product, input) {
  if (product.dtfShirt) return calculateDtfShirtLine(product, input);
...
function calculateDtfShirtLine(product, input) {
  const pack = (product.dtfPackages || []).find((item) => item.id === input.dtfPackageId);
  if (!pack) throw new Error("Pilih paket sablon kaos yang valid");
  if (!Array.isArray(input.shirtVariants) || !input.shirtVariants.length || input.shirtVariants.length > 10) {
    throw new Error("Isi minimal satu kombinasi warna dan ukuran kaos");
  }
  const seen = new Set();
  const variants = input.shirtVariants.map((row) => {
    const color = String(row.color || "");
    const size = String(row.size || "");
    const quantity = Number(row.quantity);
    if (!DTF_COLORS.includes(color) || !DTF_SIZES.includes(size) || !Number.isSafeInteger(quantity) || quantity < 1) {
      throw new Error("Warna, ukuran, atau jumlah kaos tidak valid");
    }
    const material = DTF_SHIRT_MATERIALS.find((item) => item.name === `Kaos Polos ${color} ${size}`);
    if (seen.has(material.id)) throw new Error(`Varian ${color} ${size} ditambahkan lebih dari sekali`);
    seen.add(material.id);
    return { color, size, quantity, materialId: material.id, sku: material.sku };
  });
  const quantity = variants.reduce((sum, item) => sum + item.quantity, 0);
  const baseTotal = variants.reduce((sum, item) => sum + item.quantity * (pack.price + (item.size === "XXL" ? 15000 : 0)), 0);
  return {
    productId: product.id, productName: product.name, priceBasis: "unit", width: 1, actualLength: 1, billedLength: 1,
    quantity, unitPrice: pack.price, originalUnitPrice: pack.price, discountApplied: false, baseTotal,
    finishing: [], finishingTotal: 0, fileService: { ...FILE_SERVICES[0] }, fileServiceTotal: 0,
    templateDesign: "", templateDesignTotal: 0, subtotal: baseTotal,
    dtfPackageId: pack.id, dtfPackageLabel: pack.label,
    shirtVariants: variants.map(({ color, size, quantity }) => ({ color, size, quantity })),
    materials: variants.map(({ materialId, sku, color, size, quantity }) => ({ materialId, sku, name: `Kaos Polos ${color} ${size}`, unit: "pcs", units: quantity })),
    displaySize: `${pack.label} · ${variants.map((item) => `${item.color} ${item.size} × ${item.quantity}`).join(", ")}`,
    stockSku: "", stockConsumption: quantity,
    productionNote: String(input.productionNote || "").trim()
  };
}
