  ...
  return {
    ...
    materials: [
      ...
      ...selectedChoices.map((choice) => {
        ...
        return option?.materialId ? { materialId: option.materialId, sku: option.sku, name: option.name || option.label, unit: option.unit, units: Number((Number(option.quantity || 1) * Number(fixedVariant?.choiceMaterialQuantity || 1) * quantity).toFixed(4)) } : null;
      }).filter(Boolean),
      ...selectedFinishing.map((finish) => {
        const definition = product.finishing.find((item) => item.id === finish.id);
        return definition?.materialId ? { materialId: definition.materialId, sku: definition.sku, name: definition.name, unit: definition.unit || "set", units: finish.units } : null;
      }).filter(Boolean)
    ],
