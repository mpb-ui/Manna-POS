...
export function calculateLine(product, input) {
  ...
  return {
    ...
    materials: (product.materialSources || []).map((source) => ({
      ...
      name: source.name,
      unit: source.unit,
      units: Number((Number(source.quantity || 0) * (isFixedSize ? Number(fixedVariant.area || 1) * quantity : baseUnits) * (1 + Number(source.wastePercent || 0) / 100)).toFixed(4))
    })),
    ...
    displaySize: isFixedSize ? `${fixedVariant.label} · ${quantity} Lbr` : isUnit ? `${quantity} ${product.unitName || "unit"}` : `${width} × ${billedLength} m · ${quantity} Lbr${templateDesign ? ` · ${templateDesign}` : ""}`,
