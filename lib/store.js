import pg from "pg";
import { A3_CATALOG_PRODUCTS } from "./a3-catalog.js";
import { DTF_SHIRT_MATERIALS, DTF_SHIRT_PRODUCT } from "./dtf-catalog.js";

const { Pool } = pg;

export const DEFAULT_CATALOG_OPTIONS = {
  categories: ["Outdoor", "Print A3+", "LF Poster", "LF Sticker", "Display & Banner", "Merchandise", "Sablon DTF", "ATK"],
  saleUnits: ["pcs", "lbr", "m²", "m lari", "pack", "rim", "set"],
  priceBases: [
    { id: "unit", label: "Per unit", mode: "unit" },
    { id: "sqm", label: "Luas m²", mode: "sqm" },
    { id: "linear_m", label: "Meter lari", mode: "linear_m" }
  ]
};
...
export const FINISHINGS = [
  ...
  { id: "fin-a3-laminating-a4", code: "FIN-A3-09", name: "Laminating A4", categories: ["Print A3+"], price: 6000, rule: "free", active: true },
  { id: "fin-a3-laminating-a3", code: "FIN-A3-10", name: "Laminating A3", categories: ["Print A3+"], price: 15000, rule: "free", active: true },
  { id: "fin-a3-sticker-diecut", code: "FIN-A3S-01", name: "Cutting Diecut Sticker A3+", categories: ["Print A3+"], price: 10000, rule: "free", active: true },
  { id: "fin-a3-sticker-kisscut", code: "FIN-A3S-02", name: "Cutting Kisscut Sticker A3+", categories: ["Print A3+"], price: 10000, rule: "free", active: true },
  { id: "fin-a3-sticker-knife8", code: "FIN-A3S-03", name: "Cutting Pisau MAX.8PCS Sticker A3+", categories: ["Print A3+"], price: 1500, rule: "free", active: true },
  { id: "fin-a3-sticker-transfer", code: "FIN-A3S-04", name: "Transfer Sticker A3+", categories: ["Print A3+"], price: 10000, rule: "free", active: true },
...
function seededProducts() {
  ...
  }), ...structuredClone(A3_CATALOG_PRODUCTS), structuredClone(DTF_SHIRT_PRODUCT)];
}
...
function hydrateProduct(product, materials, finishings) {
  ...
  const hydrateSource = (source) => {
    ...
    return { ...source, sku: material?.sku || source.sku, name: material?.name || source.name, unit: material?.unit || source.unit, cost: Number(material?.cost ?? source.cost ?? 0) };
  };
...
function inventoryRows(materials) {
  ...
  return materials.map((material) => ({
    ...
    category: material.category || "Lainnya", quantity: Number(material.stock || 0), minStock: Number(material.minStock || 0), unit: material.unit, updatedAt: timestamp
  }));
}
...
function syncCatalog(state) {
  ...
  state.users ||= [];
  state.auditLogs ||= [];
  state.catalogOptions ||= structuredClone(DEFAULT_CATALOG_OPTIONS);
  state.catalogOptions.categories ||= structuredClone(DEFAULT_CATALOG_OPTIONS.categories);
  state.catalogOptions.saleUnits ||= structuredClone(DEFAULT_CATALOG_OPTIONS.saleUnits);
  state.catalogOptions.priceBases ||= structuredClone(DEFAULT_CATALOG_OPTIONS.priceBases);
  for (const product of state.products || []) {
    if (product.category && !state.catalogOptions.categories.includes(product.category)) state.catalogOptions.categories.push(product.category);
    const saleUnit = product.saleUnit || product.unitName;
    if (saleUnit && !state.catalogOptions.saleUnits.includes(saleUnit)) state.catalogOptions.saleUnits.push(saleUnit);
  }
  ...
  if (previousCatalogVersion < 2) {
    ...
    for (const row of inventoryRows(state.materials)) {
      ...
      else Object.assign(current, { materialId: row.materialId, productName: row.productName, category: row.category, minStock: row.minStock, unit: row.unit });
    }
  }
  ...
  if (previousCatalogVersion < 8) {
    const newFinishingIds = new Set([
      "fin-a3-laminating-a4", "fin-a3-laminating-a3", "fin-a3-sticker-diecut",
      "fin-a3-sticker-kisscut", "fin-a3-sticker-knife8", "fin-a3-sticker-transfer"
    ]);
    for (const finish of FINISHINGS.filter((item) => newFinishingIds.has(item.id))) {
      if (!state.finishings.some((item) => item.id === finish.id)) state.finishings.push(structuredClone(finish));
    }
    for (const seeded of A3_CATALOG_PRODUCTS) {
      if (!state.products.some((item) => item.id === seeded.id)) {
        state.products.push(hydrateProduct(seeded, state.materials, state.finishings));
      }
    }
  }
  if (previousCatalogVersion < 9) {
    if (!state.catalogOptions.categories.includes("Sablon DTF")) state.catalogOptions.categories.push("Sablon DTF");
    for (const seeded of DTF_SHIRT_MATERIALS) {
      if (!state.materials.some((item) => item.id === seeded.id || item.sku === seeded.sku)) state.materials.push(structuredClone(seeded));
    }
    if (!state.products.some((item) => item.id === DTF_SHIRT_PRODUCT.id)) {
      state.products.push(hydrateProduct(structuredClone(DTF_SHIRT_PRODUCT), state.materials, state.finishings));
    }
  }
  for (const row of inventoryRows(DTF_SHIRT_MATERIALS)) {
    if (!state.inventory.some((item) => item.materialId === row.materialId || item.sku === row.sku)) state.inventory.push(row);
  }
  state.catalogVersion = 9;
  return state;
}

export function seedState() {
  const materials = [...structuredClone(MATERIALS), ...structuredClone(DTF_SHIRT_MATERIALS)];
  ...
  return {
    ...
    users: [],
    auditLogs: [],
    catalogOptions: structuredClone(DEFAULT_CATALOG_OPTIONS),
    nextOrderNumber: 1,
    catalogVersion: 9
  };
}
